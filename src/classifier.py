...
def classify(cleaned_text: str) -> dict:
    ...
    判定流程（两个维度独立判定，互不耦合）：
      risk_level（情绪严重度，NLP 同学标准）：
        1. 自杀关键词命中 → Suicidal
        2. Depression 类情绪词命中（崩溃/低落/emo/抑郁/失眠 等）→ Depression
        3. Anxiety 类情绪词命中（焦虑/紧张/担心 等）→ Anxiety
        4. 其他/零命中 → Normal
    ...
    if any(k in text for k in suicide_kws):
        risk_level = "Suicidal"
    elif any(k in text for k in risk_kws["Depression"]):
        risk_level = "Depression"
    elif any(k in text for k in risk_kws["Anxiety"]):
        risk_level = "Anxiety"
    else:
        risk_level = "Normal"
